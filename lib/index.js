...
exports.updateAreas = async (areas, maxZoom) => {
  ...
  await fs.remove('./data/last-diff.mbtiles')
  const lastDiff = await readMbtiles('./data/last-diff.mbtiles', {}, 'rwc')
  await lastDiff.startWriting()
  ...
  try {
    ...
    for (const area of areas) {
      if (!(await fs.exists('data/areas.json'))) {
        await fs.writeJson('data/areas.json', {})
      }
      const areasJson = await fs.readJson('data/areas.json')
      areasJson[area] = { start: new Date().toISOString() }
      await fs.writeJson('data/areas.json', areasJson)
      ...
      await world.putInfo(info)
      await lastDiff.putInfo(info)
      ...
      for await (const tilesPacket of areaTiles.createZXYStream()) {
        for (const tileKey of tilesPacket.toString().split('\n').map(t => t.trim()).filter(t => !!t)) {
          ...
          if (updated) {
            ...
            await lastDiff.putTilePromise(tileKey, newWorldData)
          }
        }
      }

      areasJson[area].end = new Date().toISOString()
      await fs.writeJson('data/areas.json', areasJson, { spaces: 2 })
    }
  } finally {
    await world.stopWriting()
    await lastDiff.stopWriting()
  }
}
